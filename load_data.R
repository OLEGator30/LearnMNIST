loadMNISTData <- function(dataFilename, labelsFilename) {
    # Loads the data from the MNIST dataset. The data can be downloaded from http://yann.lecun.com/exdb/mnist/
    #
    # Args:
    #   dataFilename: specifies the name of the file containing the data
    #   labelsFilename: specifies the name of the file containing the labels
    #
    # Returns:
    #   The data and the labels combined in a list data structure

    dataFileIn <- file(dataFilename, 'rb') # read the data
    ...
    dataFileIn <- file(labelsFilename, 'rb') # read the labels
    ...
    labels <- matrix(data=0, nrow=nData, ncol=1)
